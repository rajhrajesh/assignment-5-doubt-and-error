...
import JobCartItem from '../JobCartItem'
...
class AllJobs extends Component {
  ...
  getProfileData = async () => {
    ...
    if (response.ok === true) {
      ...
      const updatedData = fetchedData.profile_details.map(profile => ({
        name: profile.name,
  ...
  getJobsData = async () => {
    ...
    if (response.ok === true) {
      const fetchedData = await response.json()
      const updatedData = fetchedData.profile_details.map(eachData => ({
        id: eachData.id,
    ...
    return noOfJobs ? (
      <>
        <ul className="ul-job-item-container">
          {jobsData.map(each => (
            <JobCartItem key={each.id} item={each} />
          ))}
        </ul>
      </>
    ) : (
