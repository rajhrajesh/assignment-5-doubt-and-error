 ...
 //eslint-disable-next-line no-unused-vars
  getJobsData = async () => {
    ...
    if (response.ok) {
      ...
      const updatedDetailsData = [fetchedData.job_details].map(eachItem => ({
      ...
      const updatedSmiliarJobDetails = fetchedData.similar_jobs.map(
        eachItem => ({
          companyLogoUrl: eachItem.company_logo_url,
          id: eachItem.id,
          jobDescription: eachItem.job_description,
      ...
      this.setState({
        jobDataDetails: updatedDetailsData,
        similarJobsdata: updatedSmiliarJobDetails,
